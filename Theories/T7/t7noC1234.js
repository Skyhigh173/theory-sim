...
export function t7noC1234(data) {
  ...
  function updateMilestones() {
    ...
    if (val >= 25) dimension = 1;
    if (val >= 50) r1r2term = 1;
    if (val >= 75) r22term = 1;
  }
  ...
  function buy() {
    ...
    while (c4_c <= rho && false) {
      rho = subtract(rho, c4_c);
      c4_c += Math.log10(2.82);
      c4++;
    }
    while (c3_c <= rho && false) {
      rho = subtract(rho, c3_c);
      c3_c += Math.log10(63);
      c3++;
    }
    while (c2_c <= rho && lastPub < 50) {
      rho = subtract(rho, c2_c);
      c2_c += Math.log10(8);
      c2++;
    }
    while (c1_c <= rho && lastPub < 50) {
