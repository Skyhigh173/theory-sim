...
export function t7PlaySpqcey(data) {
  ...
  function updateMilestones() {
    ...
    if (val >= 25) r12term = 1;
    if (val >= 50) dimension = 1;
    if (val >= 75) r22term = 1;
    if (val >= 100) r1r2term = 1;
  }
  ...
  function buy() {
    ...
    while (c5_c <= rho && c5_c + Math.log10(4) <= c6_c) {
      rho = subtract(rho, c5_c);
      c5_c += Math.log10(60);
      c5++;
    }
    while (c4_c <= rho && c4_c + 1 <= c6_c) {
      rho = subtract(rho, c4_c);
      c4_c += Math.log10(2.82);
      c4++;
    }
    while (c3_c <= rho && c3_c + 1 <= c6_c) {
      rho = subtract(rho, c3_c);
      c3_c += Math.log10(63);
      c3++;
    }
    while (c2_c <= rho && lastPub < 25) {
      rho = subtract(rho, c2_c);
      c2_c += Math.log10(8);
      c2++;
    }
    while (c1_c <= rho && lastPub < 25) {
    ...
    while (q1_c <= rho && q1_c + Math.log10(4) <= c6_c) {
