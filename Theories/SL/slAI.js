...
export function slAI(data) {
  ...
  function simulate() {
    ...
    if (Math.max(lastPub, maxRho1) < 300) {
      if (rho1 + Math.log10(4) < Math.min(b1_c, b2_c)) {
        updateMilestones(Math.max(lastPub, maxRho1), 0);
      } else if (curMult > 4.5 || rho1 + Math.log10(3) > Math.min(b1_c, b2_c)) {
        updateMilestones(Math.max(lastPub, maxRho1), 1);
      } else if (
        rho1 + Math.log10(4) > Math.min(b1_c, b2_c) &&
        rho1 + Math.log10(3) < Math.min(b1_c, b2_c) &&
        lastPub < 175
      ) {
        updateMilestones(Math.max(lastPub, maxRho1), 2);
      }
    }
