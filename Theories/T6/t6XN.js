...
export function t6XN(data) {
  ...
  function simulate() {
    ...
    if (maxRho < lastPub) {
      if (coast > curMult || coast == 0) buyBR();
    } else {
      if (coast > curMult || coast == 0) buyAR();
    }
  }
  while (true) {
    simulate();
  ...
  function buyBR() {
    ...
    while (c2_c <= rho && c2_c + Math.log(2) <= Math.min(c5_c, q2_c, r2_c)) {
      rho = subtract(rho, c2_c);
      c2_c += Math.log10(5);
      c2++;
    }
    while (c1_c <= rho && c1_c + 1 <= c2_c) {
    ...
    while (q1_c <= rho && q1_c + Math.log10(5) <= Math.min(c5_c, q2_c, r2_c)) {
  ...
  function buyAR() {
    ...
    while (c2_c <= rho && c2_c + Math.log(10) <= Math.min(c5_c, q2_c, r2_c)) {
      rho = subtract(rho, c2_c);
      c2_c += Math.log10(5);
      c2++;
    }
    ...
    while (r1_c <= rho && r1_c + 1 <= Math.min(c5_c, q2_c, r2_c)) {
      rho = subtract(rho, r1_c);
      r1_c += 5;
      r1 = add(r1, log2 * Math.floor(r1lvl / 10));
      r1lvl++;
    ...
    while (q1_c <= rho && q1_c + Math.log10(8) <= Math.min(c5_c, q2_c, r2_c)) {
      rho = subtract(rho, q1_c);
      q1_c += Math.log10(3);
      q1 = add(q1, log2 * Math.floor(q1lvl / 10));
      q1lvl++;
