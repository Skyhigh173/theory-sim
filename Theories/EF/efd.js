...
export function efd(data) {
  ...
  let cap = [data.cap || 100000, 0];

  function simulate() {
    if (t == 0) startTime = performance.now();
    if (rho > maxRho) maxRho = rho;

    // if (maxRho < mscost[0] + mscost[1] * (mscost[2] - 1)) {
    if (maxRho > lastPub) {
    ...
    let maxval = Math.max(maxRho, lastPub);
  ...
  function buy() {
    ...
    while (q1_c <= rho && (q1_c + q1weight <= q2_c || q1lvl < 2)) {
