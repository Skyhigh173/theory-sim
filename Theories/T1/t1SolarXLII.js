...
export function t1SolarXLII(data) {
  ...
  while (true) {
    ...
    if (
      ...
      return [
        ...
        `T1${c4 + rhom2 * 0.3 - term1 > 3 ? "SolarXLII" : "AI"}${
          coast == 0 ? "" : coast.toFixed(1)
        }`,
        maxTauH.toFixed(7),
        convertTime(pubT),
        [pubRho, pubT],
  ...
  function buy() {
    ...
    while (
      c3_c + Math.log10(Math.min(5, lastPub / 50 + 1)) <= rho &&
      mc3term === 1
    ) {
    ...
    while (c2_c + Math.max(Math.log10(5), c4 + rhom2 * 0.3 - term1) <= rho) {
    ...
    while (
      q1_c + Math.log10(5) <= rho &&
      q1_c + q1weight <= q2_c &&
      q1_c + q1weight + Math.log10(1.5) < (mc4term > 0 ? c4_c : 1000)
    ) {
