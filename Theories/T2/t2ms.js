...
export function t2ms(data) {
  ...
  function updateMilestones(val) {
    ...
    if (val >= 75) milestones = [0, 0, 3, 0];
    if (val >= 100) milestones = [0, 0, 3, 1];
    if (val >= 125) milestones = [0, 0, 3, 2];
    if (val >= 150) milestones = [0, 0, 3, 3];
    if (val >= 175) milestones = [1, 0, 3, 3];
    if (val >= 200) milestones = [2, 0, 3, 3];
    if (val >= 225) milestones = [2, 1, 3, 3];
    if (val >= 250) milestones = [2, 2, 3, 3];
    if (milestones[0] + milestones[1] + milestones[2] + milestones[3] > msT) {
      msTimer = 40;
      state = 1;
    }
    msT = milestones[0] + milestones[1] + milestones[2] + milestones[3];
  }
  ...
  function simulate() {
    ...
    t += dt / 1.5;
    dt = Math.max(maxRho, lastPub) > 260 ? dt * 1.0001 : 1;

    drho = maxRho - lastPub;
    tauH = Math.max(0, drho / (t / 3600));
    if (maxTauH < tauH || pubRho < 15) {
      maxTauH = tauH;
      pubDRho = drho;
      maxT = t * 2;
      pubT = t;
    ...
    if (msT == 1 || msT == 2) {
      if (msTimer == 10 && state == 0) {
        milestones = [msT, 0, 0, 0];
        state = 1;
        msTimer = 0;
      } else if (msTimer == 40 && state == 1) {
        milestones = [0, 0, msT, 0];
        state = 2;
        msTimer = 0;
      } else if (msTimer == 5 && state == 2) {
        milestones = [0, msT, 0, 0];
        state = 3;
        msTimer = 0;
      } else if (msTimer == 40 && state == 3) {
        milestones = [0, 0, msT, 0];
        state = 0;
        msTimer = 0;
      }
    ...
    if (msT == 5 || msT == 6) {
      if (msTimer == 10 && state == 0) {
        milestones = [2, 2, msT - 4, 0];
        state = 1;
        msTimer = 0;
      } else if (msTimer == 40 && state == 1) {
        milestones = [0, 0, 3, msT - 3];
        state = 0;
        msTimer = 0;
      }
      msTimer++;
    ...
    if (msT > 6) {
      if (msTimer == 10 && state == 0) {
        milestones = [2, 2, 3, msT - 7];
        state = 1;
        msTimer = 0;
      } else if (msTimer == 40 && state == 1) {
        milestones = [
          Math.min(2, msT - 6),
          msT - 6 - Math.min(2, msT - 6),
          3,
          3,
        ];
        state = 0;
