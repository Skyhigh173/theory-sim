...
export function t4(data) {
  ...
  function simulate() {
    ...
    let vc2 = log2 * c2;
    let vc3 = log2 * c3;
    let vc4 = mterms > 0 ? Math.log10(3) * c4 : 0;
    let vc5 = mterms > 1 ? Math.log10(5) * c5 : 0;
    let vc6 = mterms > 2 ? c6 : 0;
    ...
    let rhodot =
      ...
      Math.log10(dt) +
      add(
        add(add(add(vc1 + vc2, vc3 + q), vc4 + q * 2), vc5 + q * 3),
        vc6 + q * 4
      );
    rho = add(rho, rhodot);
  ...
  function buy() {
    ...
    while (c6_c <= rho && mterms > 2) {
      rho = subtract(rho, c6_c);
      c6_c += Math.log10(58);
      c6++;
    }
    while (c5_c <= rho && mterms > 1) {
      rho = subtract(rho, c5_c);
      c5_c += Math.log10(12.5);
      c5++;
    }
    while (c4_c <= rho && mterms > 0) {
      rho = subtract(rho, c4_c);
      c4_c += Math.log10(4.85);
      c4++;
    }
    while (c3_c <= rho) {
