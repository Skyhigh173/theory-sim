...
export function t5AI(data) {
  ...
  let switchPoint = (lastPub / 4) ** 1.1 - 100;
  ...
  function buy() {
    ...
    while (c2_c <= rho && (c2_c + Math.log10(8) < maxRho || t >= switchPoint)) {
    ...
    while (
      c1_c <= rho &&
      (c1_c + Math.min(4, curMult * (lastPub ** 1.02 / 25)) <
        Math.min(q2_c, c2_c, c3_c) ||
        t <= switchPoint)
    ) {
    ...
    while (
      q1_c <= rho &&
      (q1_c + q1weight <= Math.min(q2_c, c2_c, c3_c) || t <= switchPoint)
    ) {
